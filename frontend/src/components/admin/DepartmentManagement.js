...
const DepartmentManagement = () => {
  ...
  const canManageDepartments = user && ['Admin', 'Vice President', 'HR BP', 'HR Manager', 'HR Executive', 'Team Manager', 'Team Leader'].includes(user.role);
  ...
  const handleSubmit = async (e) => {
    ...
    // Client-side validation for department code format
    if (formData.code && !/^[A-Z0-9]+$/.test(formData.code)) {
      setError('Department code must contain only uppercase letters and numbers');
      return;
    }
    ...
    try {
      ...
      if (errorData?.errors && Array.isArray(errorData.errors)) {
        const validationErrors = errorData.errors.map(error => error.msg).join(', ');
        setError(validationErrors);
      } else {
        // Handle other error messages
        setError(errorData?.message || err.message || 'Failed to save department');
      }
    }
  };
  ...
  const handleDelete = async (departmentId) => {
    ...
    try {
      ...
      const errorData = err.response?.data;
      setError(errorData?.message || err.message || 'Failed to delete department');
    }
  };
  ...
  const handleInputChange = (e) => {
    ...
    // Auto-convert department code to uppercase and validate format
    if (name === 'code') {
                    ...
                    <div className="col-md-6">
                      <div className="mb-3">
                        ...
                        <input
                          ...
                          required
                        />
                        <div className="form-text">Only uppercase letters and numbers allowed (max 10 characters)</div>
                      </div>
                    </div>
                  </div>
